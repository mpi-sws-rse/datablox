...
class restore_manager(Element):
  def on_load(self, config):
    ...
    self.add_port("chunk_index", Port.PULL, Port.UNNAMED, ["fingerprint"])
    self.add_port("meta_index", Port.PULL, Port.UNNAMED, ["path"])
  
  def fetch_file_fps(self, path):
    log = Log()
    log.append_field("path", [path])
    fps_log = self.pull("meta_index", log)
  ...
  def fetch_chunks(self, fps):
    ...
    clog = self.pull("chunk_index", log)
